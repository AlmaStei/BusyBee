...
## Netting - no outliers POISSON

Since the DHARMa package flagged outliers as a significant issue, we will try to remove them and re-run the model.

```{r outlier removal}
#define the outliers according to the DHARMa package
outliers <- which(abs(residuals(netting_mod4_full, type = "pearson")) > 3)



netting2 <- netting1 %>%
  #remove main outlier - interaction count over 300
  filter(total_interaction_T < 300)
  #remove DHARMa outliers
  #filter(!row_number() %in% outliers)

netting3 <- netting1 %>%
  #substract 100 from the outlier
  mutate(total_interaction_T = ifelse(total_interaction_T > 300, total_interaction_T - 100, total_interaction_T))
```

```{r netting2_mod1_full, fig.width=8, fig.height=10}
netting2_mod1_full <- glmer(total_interaction_T 
                                   ~ Floral_simpson_index 
                                   + Time_category
                                   + top2_ratio
                                   + Site_type
                                   ...
                                   + Days_since_start
                                   + (1|site), 
                                   data = netting2, family = "poisson")
summary(netting2_mod1_full)
parameters(netting2_mod1_full)

#check the model
check_model(netting2_mod1_full, verbose = T)

#overdispersion
check_overdispersion(netting2_mod1_full)
#collinearity
check_collinearity(netting2_mod1_full)

# Compute fitted values and Pearson residuals
#netting2_mod1_vals <- fitted(netting2_mod1_full)
#netting2_mod1_residuals <- residuals(netting2_mod1_full, type = "pearson")
# Create binned residuals plot
#arm::binnedplot(netting2_mod1_vals, netting2_mod1_residuals)

#DHARMa package - simulate residuals and check model assumptions
netting2_mod1_sim_res <- simulateResiduals(fittedModel = netting2_mod1_full)
plot(netting2_mod1_sim_res)

```
```{r netting2_mod2_full, fig.width=8, fig.height=10}
# removing floral simpson index
netting2_mod2_full <- glmer(total_interaction_T 
                                   ~ Time_category
                                   #Floral_simpson_index
                                   + top2_ratio
                                   + Site_type
...
summary(netting2_mod2_full)
parameters(netting2_mod2_full)

#check the model
check_model(netting2_mod2_full, verbose = T)

#overdispersion
check_overdispersion(netting2_mod2_full)
#collinearity
check_collinearity(netting2_mod2_full)

# Compute fitted values and Pearson residuals
# netting2_mod2_vals <- fitted(netting2_mod2_full)
# netting2_mod2_residuals <- residuals(netting2_mod2_full, type = "pearson")
# Create binned residuals plot
# arm::binnedplot(netting2_mod2_vals, netting2_mod2_residuals)

#DHARMa package - simulate residuals and check model assumptions
netting2_mod2_sim_res <- simulateResiduals(fittedModel = netting2_mod2_full)
plot(netting2_mod2_sim_res)
```

```{r netting2_mod3_full, fig.width=8, fig.height=10}
# removing days since start
netting2_mod3_full <- glmer(total_interaction_T 
                                   ~ Time_category
                                   #+ Floral_simpson_index
                                   + top2_ratio
                                   + Site_type
...
summary(netting2_mod3_full)
parameters(netting2_mod3_full)

#check the model
check_model(netting2_mod3_full, verbose = T)
#overdispersion
check_overdispersion(netting2_mod3_full)
#collinearity
check_collinearity(netting2_mod3_full)

# Compute fitted values and Pearson residuals
# netting2_mod3_vals <- fitted(netting2_mod3_full)
# netting2_mod3_residuals <- residuals(netting2_mod3_full, type = "pearson")
# Create binned residuals plot
# arm::binnedplot(netting2_mod3_vals, netting2_mod2_residuals)

#DHARMa package - simulate residuals and check model assumptions
netting2_mod3_sim_res <- simulateResiduals(fittedModel = netting2_mod3_full)
plot(netting2_mod3_sim_res)
```

```{r netting2_mod4_full, fig.width=8, fig.height=10}
# removing wind velocity
netting2_mod4_full <- glmer(total_interaction_T 
                                   ~ Time_category
                                   #+ Floral_simpson_index
                                   + top2_ratio
                                   + Site_type
...
summary(netting2_mod4_full)
parameters(netting2_mod4_full)

#check the model
check_model(netting2_mod4_full, verbose = T)

#overdispersion
check_overdispersion(netting2_mod4_full)
#collinearity
check_collinearity(netting2_mod4_full)

# Compute fitted values and Pearson residuals
# netting2_mod4_vals <- fitted(netting2_mod4_full)
# netting2_mod4_residuals <- residuals(netting2_mod4_full, type = "pearson")
# Create binned residuals plot
# arm::binnedplot(netting2_mod4_vals, netting2_mod4_residuals)

#DHARMa package - simulate residuals and check model assumptions
netting2_mod4_sim_res <- simulateResiduals(fittedModel = netting2_mod4_full)
plot(netting2_mod4_sim_res)
...
```{r}
# Compare the models with the performance package
net_pois_comp2 <- compare_performance(netting2_mod1_full, netting2_mod2_full, netting2_mod3_full, netting2_mod4_full,  
                                        metrics = c("AICc", "BIC", "R2", "ICC", "RMSE"))

# Print the comparison table
print(net_pois_comp2)
```
...
## NB REMOVING OUTLIERS

```{r netting_mod1_nb, fig.width=8, fig.height=10}
# full model with interaction counts as response variable and environmental, weather and plant diversity variables as explanatory variables, and site as random effect
# negative binomial distribution

netting2_mod1_NB <- glmer.nb(total_interaction_T 
                            ~ Floral_simpson_index 
                            + Time_category
                            + top2_ratio
                            + Site_type
...
summary(netting2_mod1_NB)
parameters(netting2_mod1_NB)

#check the model
check_model(netting2_mod1_NB, verbose = T)
#overdispersion
check_overdispersion(netting2_mod1_NB)
#collinearity
check_collinearity(netting2_mod1_NB)

# Compute fitted values and Pearson residuals
netting2_mod1_NB_vals <- fitted(netting2_mod1_NB)
netting2_mod1_NB_residuals <- residuals(netting2_mod1_NB, type = "pearson")
# Create binned residuals plot
arm::binnedplot(netting2_mod1_NB_vals, netting2_mod1_NB_residuals)

#DHARMa package - simulate residuals and check model assumptions
netting2_mod1_NB_sim_res <- simulateResiduals(fittedModel = netting2_mod1_NB)
plot(netting2_mod1_NB_sim_res)
...
```{r netting_mod1_tweedie, fig.width=8, fig.height=10}
netting_mod_TW <- glm(total_interaction_T 
                      ~ Floral_simpson_index 
                      + Time_category
                           + top2_ratio
                           + Site_type
...
summary(netting_mod_TW)

plot(netting_mod_TW)  # Plots model diagnostics
gam.check(netting_mod_TW)  # Checks for goodness of fit

str(netting1)  # Check structure
summary(netting1)  # Look for missing values
any(is.na(netting1))  # Check if there are NA values
```
