...
The goal of this script is to clean and standardize plant data collected from 9 sites in Sachsen-Anhalt. 
The resulting variables are: Site, Site_type, Plant_sp, Transect, Plot, Cover_percentage, BB (Braun-Blanquet scale).
This is then saved as a .csv file with the cleaned plant data.

One of the major fixes applied to this dataset is the transformation of a mix between cover percentage and Braun-Blanquet scale, to two consistent columns: one with percentages of flower cower and one with categories following the Braun-Blanquet scale. 

After all this, we calculate plant species richness, and estimate plant species diversity using rarefaction curves. The script also performs NMDS analysis to visualize plant species composition between sites and conducts a PERMANOVA test to determine whether plant communities significantly differ between restored and reference sites. The script also aggregates plant data with pollinator interaction data to explore the relationship between flower abundance (cover %) and pollinator interactions.
...
5. The remaining cells in Cover_percentage are still empty (NA), and filled with the equivalent BB values (r -> 0.3%, r -> 0.75%).
...
```{r save plants3 data}
# Save the cleaned plant data in a wide format

#create new column combining the site names, transect and plot
plants3$Site_Tn_Pn <- paste(plants3$Site, plants3$Transect, plants3$Plot, sep = "_")

# wide format filling with Cover_percentage
plants3_wide <- plants3 %>%
  select(-BB)%>%
  pivot_wider(names_from = Plant_sp, values_from = Cover_percentage, values_fill = 0)

# Save the wide and cleaned plant data
write.csv(plants3_wide, "C:/Users/Almas/Desktop/UNI_LEIPSI/Thesis/Thesis_Rproject/data/plants3.csv", row.names = FALSE)
rm(plants3_wide)
```
