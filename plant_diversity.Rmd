...
community_matrix <- plants3 %>%
  ...
  summarise(Cover_percentage = sum(Cover_percentage), .groups = "drop") %>%
...
ggplot(richness, aes(x = Site, y = Species_Richness, color = Site_type)) +
  ...
  ylim(0, 30) +
  #custom colors
  scale_color_manual(values = steelorange)
```

```{r}
colnames(plants3)

diversity_scores <- plants3 %>%
  select(-BB) %>%
  ...
  summarise(
    ...
    # You could also calculate a weighted Simpson Index or other diversity metrics as needed
    .groups = 'drop'  # Ensures results are ungrouped
  )


ggplot(diversity_scores, aes(x = Site, y = Shannon_Index, fill = Transect)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(
    title = "Plant Diversity Gradient per Transect",
    x = "Site",
    y = "Shannon Index",
    fill = "Transect")+
    scale_fill_viridis(discrete = TRUE)
...
ggplot(diversity_scores, aes(x = Site, y = Species_Richness, fill = Transect)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(
    title = "Plant Diversity Gradient per Transect",
...
#adding the gradient of plant diversity to the pol data 
...
dim(diversity_scores)
dim(poli)

#add the diversity scores to the pollinator interaction data
pol_diversity <- left_join(poli, diversity_scores, by = c("Site", "Transect"))

#scatterplot of Species richness vs. Total Interactions, custom colors for the sites
ggplot(pol_diversity, aes(x = Species_Richness, y = total_interaction)) +
...
ggplot(pol_diversity, aes(x = Shannon_Index, y = total_interaction)) +
  geom_point(aes(color = Site)) +
  geom_smooth(method = "lm", se = T, color = "blue") +
...
## F. Aggregate plant data and pollinator interaction data
```{r}
...
rm(flower_cover_summary)  # Remove intermediate dataframes to save memory
```
...
print(cor_test)
```
### 3 most pollinator visited plant species
...
```
### abundance of the 4 most visited plant species

```{r top4 creation and save csv}
top4 <- plants3%>%
  #remove NA rows in Cover_percentage
  filter(!is.na(Cover_percentage)) %>%
  #keep only rows that are top 4 plant species: Daucus carota, Pastinaca sativa, Peucedanum officinale and Galium album
  filter(Plant_sp %in% c("Daucus carota", "Pastinaca sativa", "Peucedanum officinale", "Galium album")) %>%
  #make wider with Plant_sp as column names
  pivot_wider(names_from = Plant_sp, values_from = Cover_percentage, values_fill = 0) %>%
  #add the percentages of the 4 plant species into a column named top4_percentage
  rowwise() %>%
  mutate(top4_percentage = sum(c(`Daucus carota`, `Pastinaca sativa`, `Peucedanum officinale`, `Galium album`)))%>%
  #summarise the mean top4_coverage per transect 
  group_by(Site, Transect) %>%
  summarise(mean_top4_coverage = mean(top4_percentage, na.rm = TRUE))

#save the top4 data as csv
write.csv(top4, "C:/Users/Almas/Desktop/UNI_LEIPSI/Thesis/Thesis_Rproject/data/top4.csv", row.names = FALSE)
```
...
```{r network metrics, eval=FALSE, include=FALSE}
# H2 --------------------------------------------------
# Reference sites
