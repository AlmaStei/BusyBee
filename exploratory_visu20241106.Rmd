...
ggplot(insect_large, aes(x = Taxa, y = Count, fill = Site)) +
  ...
  scale_fill_viridis(discrete = TRUE, option= "plasma") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100), n.breaks =8)
...
ggplot(interaction_counts, aes(x = reorder(site, interaction_count), y = interaction_count, fill = family)) +
  geom_bar(stat = "identity") +
...
str(net_data) #check the structure of the data

length(net_data$final_ID) 
# 1579 

net_data <- net_data %>%
  filter(plant_species != "no info") #remove rows with no info in the plant_species column  
