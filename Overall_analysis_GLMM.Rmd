...
platform_camera1 <- platform_camera1 %>%
 #scale these columns: "dm_wind_velocity" "dm_temperature" "agri" "grass" "snh" "forest" "urban" "water" "Pastinaca.sativa" "Daucus.carota" "top2_ratio" "Floral_simpson_index" "minutes_since_9am"  
  mutate(across(c("dm_wind_velocity", "dm_temperature", "agri", "grass", "snh", "forest", "urban", "water", "Pastinaca.sativa", "Daucus.carota", "top2_ratio", "Floral_simpson_index", "minutes_since_9am", "Days_since_start"), scale))
...
plantsimp_mod1_full <- lmer(Floral_simpson_index 
                            ~ Site_type
                            +majority_class 
...
```{r fig.width=8, fig.height=10}
plot_model(plantsimp_mod1_full , type = "est", show.values = TRUE, value.offset = .3)
```
...
- Minutes since 9 am
    + Time since 9 am, when the netting started, to account for the time of day. Each netting session is made to be 30 minutes of active netting, so the sampling effort is the same for all transects. 
- Top 2 ratio
    + Relative abundance ratio of D. carota to P. sativa used as a proxy for dominance shifts.
- Site type
...
# full model with interaction counts as response variable and environmental, weather and plant diversity variables as explanatory variables, and site as random effect
# removing time category (p =0.468 in the previous model)
...
```{r netting_pois_comp1}
# Compare the models with the performance package
netting_pois_comp1 <- compare_performance(netting_mod1_full, netting_mod2_full, netting_mod3_full, netting_mod4_full,  
                                        metrics = c("AICc", "BIC", "R2", "ICC", "RMSE"))

# Print the comparison table
print(netting_pois_comp1)
```
...
# full model with interaction counts as response variable and environmental, weather and plant diversity variables as explanatory variables, and site as random effect
# remove minutes_since_9am (p= 0.432 for netting_mod1_NB)
...
The comparison of the models shows that the model with the lowest AICc and BIC is the fifth model (netting_mod5_NB), which has the lowest AICc and BIC values. However, the model was flagged as singular (*boundary (singular) fit: see help('isSingular')*), indicating that the random effects structure was overfitted. Here, the variance estimate for the random intercept is effectively zero:    
|Parameter            | Coefficient|
|---------------------|------------|
|SD (Intercept: site) |        0.00|

indicating that site-level variation did not contribute meaningfully to the model. This suggests that the fixed effects sufficiently explained the variation in total interactions across sites, and that the random effect of site can be removed without significantly affecting the model fit.
...
```{r}
mod_with_random <- glmer.nb(total_interaction_T ~ Floral_simpson_index + top2_ratio + Site_type + dm_wind_velocity + (1 | site), data = netting1)
mod_without_random <- glm.nb(total_interaction_T ~ Floral_simpson_index + top2_ratio + Site_type + dm_wind_velocity, data = netting1)

anova(mod_with_random, mod_without_random)
``` 

We compared the model with random effect to one without a random intercept for site using a likelihood ratio test. The comparison revealed no difference in model fit (χ²(1) = 0, p = 1), with identical log-likelihoods and deviance. AIC also slightly favored the simpler model. We therefore removed the random effect to improve model parsimony without sacrificing model quality.

#### Visualize the model results

```{r}
plot(ggpredict(netting_mod4_NB, terms = "top2_ratio"))
plot(ggpredict(netting_mod4_NB, terms = "dm_wind_velocity"))
#plot(ggpredict(netting_mod4_NB, terms = "Floral_simpson_index"))
...
ggplot(netting1, aes(x = Site_type, y = total_interaction_T)) +
  geom_boxplot(fill = "lightblue") +
  theme_minimal() +
  labs(y = "Total interactions", x = "Site Type")

ggplot(netting1, aes(x = majority_class, y = total_interaction_T)) +
  geom_boxplot(fill = "lightblue") +
  theme_minimal() +
  labs(y = "Total interactions", x = "Majority landcover class in 1km radius around sites")
```
...
pred_wind <- ggpredict(netting_mod5_NB , terms = "dm_wind_velocity")
...
#removing minutes_since_9am (p= 0.443 for netting_mod1_NB_noRE)
...
#removing days since start ( p= 0.341 for netting_mod2_NB_noRE)
...
#removing temperature (p= 0.229  for netting_mod3_NB_noRE)
...
# removing floral_simpson_index (p= 0.407 ,  for netting_rich_mod4_poiss)
...
mod_without_random <- glm(unique_taxa ~  Floral_simpson_index 
                                 + minutes_since_9am
                                 + top2_ratio
                                 #+ Site_type
                                 #+ dm_wind_velocity
...
# removing floral_simpson_index (p= 0.407    ,  for netting_rich_mod4_noRE_poiss)
...
## B.2.1. RICHNESS - Platform cameras - POISSON glmer (offset: recording time)
...
#remove floral simpson index (p= 0.543   for platform_richness_mod1_poiss)
...
#remove top2_ratio (p= 0.556   for platform_richness_mod2_poiss)
...
#remove Site type (p= 0.49905 for platform_richness_mod3_poiss)
...
#remove Days since start (p= 0.139    for platform_richness_mod4_poiss)
...
# Compare the models with the performance package
platform_richness_poiss_comp1 <- compare_performance(platform_richness_mod1_poiss, platform_richness_mod2_poiss, platform_richness_mod3_poiss, platform_richness_mod4_poiss, platform_richness_mod5_poiss, metrics = c("AICc", "BIC", "R2", "ICC", "RMSE"))
...
# Compute fitted values and Pearson residuals
platform_shannon_mod1_gauss_vals <- fitted(platform_shannon_mod1_gauss)
platform_shannon_mod1_gauss_res <- residuals(platform_shannon_mod1_gauss, type = "pearson")

plot(platform_shannon_mod1_gauss_vals, platform_shannon_mod1_gauss_res)  # Residuals vs Fitted values plot
abline(h = 0, col = "red")  # Add a horizontal line at 0 for reference
# Create binned residuals plot
arm::binnedplot(platform_shannon_mod1_gauss_vals, platform_shannon_mod1_gauss_res)
```
...
```{r platform_shannon_mod1_glmer}
glmer(count ~ Floral_simpson_index 
      + rec_time_min
      + top2_ratio
      + Site_type
      + Days_since_start
...
#scale rec_time_min
platform_diversity <- platform_diversity %>%
  mutate(rec_time_min = scale(rec_time_min))
...
temp_sd <- sd(envir_data$dm_temperature, na.rm = TRUE)
# Get predictions on the scaled variable
pred_temp <- ggpredict(platform_shannon_mod5_gauss , terms = "dm_temperature")
# Unscale the x-axis
pred_temp$x_unscaled <- (pred_temp$x * temp_sd) + temp_mean
# Plot
ggplot(pred_temp, aes(x = x_unscaled, y = predicted)) +
  ...
  labs(
    ...
    x = "Temperature (C)",
    y = "Predicted Shannon Diversity per minute of\n
     recording on platform cameras"
  ) +
  theme_minimal()+
  #add original data points
...
# Get predictions on the scaled variable
pred_days <- ggpredict(platform_shannon_mod5_gauss , terms = "Days_since_start")
# Unscale the x-axis
