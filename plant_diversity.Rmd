...
theme_minimal() +
  labs(title = "NMDS of Plant Species by Site",
       x = "NMDS1",
       y = "NMDS2",
...
# Reorder Site levels by mean Floral_species_richness
diversity_indices <- diversity_indices %>%
  mutate(Site = fct_reorder(Site, Floral_species_richness, .fun = mean, .desc = FALSE))
...
(simp <- ggplot(diversity_indices, aes(x = Site, y = Floral_simpson_index, fill = Site)) +
 ...
 #theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(
    title = "Floral Simpson Diversity Index",
    x = "Site",
    y = "Simpson Diversity Index",
    fill = "Site"
  ) +
...
(shann <- ggplot(diversity_indices, aes(x = Site, y = Floral_shannon_index, fill = Site)) +
 ...
 #theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(
    title = "Floral Shannon Diversity Index",
    x = "Site",
    y = "Shannon Diversity Index",
    fill = "Site"
  ) +
...
ggsave("C:/Users/Almas/Desktop/UNI_LEIPSI/Thesis/Thesis_Rproject/figures/plant_diversity_metrics.png", width = 16, height = 8)
