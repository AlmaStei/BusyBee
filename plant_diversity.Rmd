...
steelorange <- c("Young Restored" = "#9BB655FF", "Reference" = "#1F78B4")
steelorange1 <- c("Young Restored" = "#C8DC9D", "Reference" = "#8DB4D9")
...
pastel_colors <- c(
  "DES" = "#CFE4F1",  # lighter pastel blue
  "HLI" = "#A9C4D6",  
  "JEP" = "#7FA1B4",  
  "STP" = "#557788",  
  "WUP" = "#8DB4D9",  
...
plants <- plant_data_load %>%
  ...
  dplyr::select(-notes)%>% #remove notes column
...
(nmdsflower <- ggplot(site_scores, aes(x = NMDS1, y = NMDS2, color = Site_Type, label = Site)) +
  ...
  labs(title = "NMDS of Flower Species by Site",
       x = "NMDS1",
       y = "NMDS2",
       color = "Site Type")+
...
(rarefaction_plant_pooled <- ggiNEXT(iNEXT_output_pooled, 
  ...
  labs(y= "Flower species diversity", x="Number of plots")+
    scale_color_manual(values = steelorange) + # Assign custom colors to sites (curves)
    scale_fill_manual(values = steelorange1) + # Assign custom colors to sites (confidence intervals)
    #scale_shape_manual(values = c(0, 1, 2, 3, 4, 5, 6, 7, 8))+  # Assign different shapes for each site
  #theme_minimal() +
    theme(legend.position = "bottom")
  )
...
group_by(Site, Site_type, Transect, Plant_sp) %>%
  ...
  ungroup()%>%
  select(-Plot, -Relative_cover_plot, -Cover_percentage) %>%
...
ggplot(relative_flower, aes(x = reorder(Plant_sp, Plot_Cover), y = Plot_Cover, fill = Site)) +
  geom_bar(stat = "identity") +
#theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(
    title = "Plot Cover % per Species per Transect",
    x = "Plant Species",
    y = "Plot cover",
    fill = "Site") +
  scale_fill_manual(values = custom_colors) +  # Apply custom colors to the plot
...
Additionally, for the flower cameras, we will need to have plant metrics but at a **site-level**. Here, we will add a column for the average cover of flower per site, and then we will calculate the average flower cover ratio per species per site. sum of plot_cover per transect divided by 5 (number of transect per site).

$$\sum_{site} \text{Plot_Cover}/5$$


```{r relative cover ratios per site}
# Calculate the average cover ratio per site
site_avg_cover <- relative_flower %>%
  group_by(Site) %>%
  summarise(average_flower_cover = mean(Plot_Cover, na.rm = TRUE))
```
...
diversity_indices <- relative_flower_w %>%
  mutate(
    Floral_simpson_index = vegan::diversity(.[, -(1:3)], index = "simpson"),
    Floral_shannon_index = vegan::diversity(.[, -(1:3)], index = "shannon")
  ) %>%
  select(1:3, Floral_simpson_index, Floral_shannon_index)  # Keep only relevant columns
...
#add site_avg_cover to the relative_flower data
relative_flower <- left_join(relative_flower, site_avg_cover, by = "Site")
...
```{r scatterplot diversity indices}
...
```{r boxplot diversity indices}
# Order the Site factor: group by Site_type first, then sort by mean Floral_species_richness within each group
site_order <- diversity_indices %>%
  group_by(Site_type, Site) %>%
  summarize(mean_richness = mean(Floral_species_richness, na.rm = TRUE)) %>%
  arrange(Site_type, mean_richness) %>%
  pull(Site)

# Apply the custom order
diversity_indices$Site <- factor(diversity_indices$Site, levels = site_order)

# Boxplot: species richness per site
(sp <- ggplot(diversity_indices, aes(x = Site, y = Floral_species_richness, fill = Site)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) + 
  #geom_jitter(aes(color = Site), width = 0.2, size = 1.5, alpha = 0.5) +
  #scale_color_manual(values = custom_colors) +
#theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(
    title = "Floral Species \nRichness",
    x = "Site",
    y = "Number of Flower Species per Transect",
    fill = "Site"
  ) +
  ylim(0, 30) +
...
# Boxplot: simpson index per site
(simp <- ggplot(diversity_indices, aes(x = Site, y = Floral_simpson_index, fill = Site)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  #geom_jitter(aes(color = Site), width = 0.2, size = 1.5, alpha = 0.5) +
  #scale_color_manual(values = custom_colors) +
 #theme_minimal() +
...
# Boxplot: shannon index per site
(shann <- ggplot(diversity_indices, aes(x = Site, y = Floral_shannon_index, fill = Site)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  #geom_jitter(aes(color = Site), width = 0.2, size = 1.5, alpha = 0.5) +
  #scale_color_manual(values = custom_colors) +
 #theme_minimal() +
...
# Combine the plots
combined_plot <- sp  + shann + simp
#+ plot_annotation(title = "Relative Flower Diversity Metrics") &  theme(plot.title = element_text(hjust = 0.5, size = 16)) #, face = "bold"

# Print and save
print(combined_plot)

library(cowplot)
cowplot::plot_grid(nmdsflower, combined_plot,ncol = 1, labels = c("A", "B"), label_size = 16, rel_widths = c(1, 1.2))

ggsave("C:/Users/Almas/Desktop/UNI_LEIPSI/Thesis/Thesis_Rproject/figures/plant_diversity_metrics.png", 
       width = 10, height = 10, dpi = 600)

#rm(sp, simp, shann, combined_plot, site_order)
```
...
top_plants$fill_color <- ifelse(top_plants$Plant_sp == "Daucus carota", "#A9CCE3",
                          ifelse(top_plants$Plant_sp == "Pastinaca sativa", "#DABD61FF", "#A3A380"))

#Plot the top 10 plant species
ggplot(top_plants, aes(x = reorder(Plant_sp, total_interactions), 
                       y = total_interactions, 
                       fill = fill_color)) +
  geom_bar(stat = "identity", width = 1, color ="black", size= 0.2) +
  scale_fill_identity() +
  labs(title = "Netting: Most Visited Flower Species",
       x = "Plant Species",
...
top2 <- relative_flower %>%
  ...
  select(-Floral_species_richness, -Floral_simpson_index, -Floral_shannon_index, -Plot_Cover) %>%
  ...
  # Pivot wider, using `values_fn` to handle missing values
  pivot_wider(
    names_from = Plant_sp,
    values_from = Relative_cover_transect,
    values_fill = 0,
    values_fn = list(Relative_cover_transect = sum)
  ) %>%
