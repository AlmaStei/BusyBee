...
```{r save long format data}
# Save the data in long format
#current_date <- format(Sys.Date(), "%Y%m%d")
# save a RData file
#save(bowltrap_clean, file = paste0("C:/Users/Almas/Desktop/UNI_LEIPSI/Thesis/Thesis_Rproject/data/", current_date, "_full_bowltrap.RData"))
```
...
net_data <- net_data %>%
        ...
        # Create a new column that defines the lowest taxonomic level available
        mutate(lowest_taxa = case_when(
               !is.na(species) ~ species, # Species level
               !is.na(genus) ~ paste("Genus:", genus), # Genus level
               TRUE ~ paste("Family:", family)))%>% # Family level
  
        # Standardize the lowest_taxa column to unify textual variations
        mutate(lowest_taxa= case_when(
               lowest_taxa %in% c("Genus: aff. Syrphus", "Genus: Syrphus") ~ "Genus: Syrphus",
               lowest_taxa %in% c("Genus: aff. Eupeodes", "Genus: Eupeodes") ~ "Genus: Eupeodes",
               lowest_taxa %in% c("fabriciana adippe") ~ "Fabriciana adippe",
               TRUE ~ lowest_taxa)) %>% # Keep other lowest_taxa unchanged
