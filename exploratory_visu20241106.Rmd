...
```{r rarefaction curves for site_type}
...
# (q = 0), Shannon diversity (q = 1, the exponential of Shannon entropy) and Simpson diversity (q = 2, the inverse of Simpson concentration).
# Source: https://cran.r-project.org/web/packages/iNEXT/vignettes/Introduction.html
set.seed(2022);iNEXT_output_insects <- iNEXT(matrix_netting_type,  q = c(0,1,2), datatype = 'abundance')
# Have a look at the output of iNext. It gives diversity estimates per meadow type.
...
```{r rarefaction curves for sites}
net_data <- as.data.frame(net_data) #make 'dat' into a data frame that can be read by create.matrix
...
# Compute and visualize diversity by number of individuals.

# iNEXT focuses on three measures of Hill numbers of order q: species richness
# (q = 0), Shannon diversity (q = 1, the exponential of Shannon entropy) and Simpson diversity (q = 2, the inverse of Simpson concentration).
# Source: https://cran.r-project.org/web/packages/iNEXT/vignettes/Introduction.html
set.seed(2022); iNEXT_output_insects <- iNEXT(matrix_netting_site,  q = c(0,1,2), datatype = 'abundance')
# Have a look at the output of iNext. It gives diversity estimates per site.
# This are specifically the diversity estimates with their confidence intervals:
iNEXT_output_insects$AsyEst 
# Can also plot the results
ggiNEXT(iNEXT_output_insects, type = 1, color.var = 'Assemblage',facet.var="Order.q")
...
#### Transects 
```{r rarefaction curves for transect number}
net_data <- as.data.frame(net_data) #make 'dat' into a data frame that can be read by create.matrix
...
# (q = 0), Shannon diversity (q = 1, the exponential of Shannon entropy) and Simpson diversity (q = 2, the inverse of Simpson concentration).
# Source: https://cran.r-project.org/web/packages/iNEXT/vignettes/Introduction.html
set.seed(2022); iNEXT_output_insects <- iNEXT(matrix_netting_transect,  q = c(0,1,2), datatype = 'abundance')
# Have a look at the output of iNext. It gives diversity estimates per site.
# This are specifically the diversity estimates with their confidence intervals:
iNEXT_output_insects$AsyEst 
# Can also plot the results
